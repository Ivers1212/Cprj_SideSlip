...
/* A command operation sequence includes:
         A command code (for serial port mode, two synchronization codes are
   required before the command code), Several input data (can be 0), Several
   output data (can be 0) Rule for naming command codes: CMDxy_NAME x indicates
   the minimum number of input data (bytes), y indicates the minimum number of
   output data (bytes). y If it is W, Indicates that you need to wait until the
   command is successfully executed Some commands can read and write data blocks
   of 0 to more than one byte, the number of bytes of the data block itself is
   not included in the x or y above*/
/* *********************************************************************************************************************
 */
#define CMD01_GET_IC_VER                                                       \
  0x01 /* Obtain the chip and firmware version number                          \
        */
       /* Output: Version number*/

#define CMD31_SET_BAUDRATE                                                     \
  0x02 /* Serial port: Set the baudrate for serial port communication (The     \
          default baudrate after power-on or reset is 9600bps */
       /* Input: 3-bytes baud rate, low bytes first */
/* Output: operation status (CMD_RET_SUCCESS, other values indicate operation
 * incomplete) */
...
#define CMD11_CHECK_EXIST                                                      \
  0x06 /* Test communication interface and working condition */
       /* Input: Any data */
       /* Output: bitwise inverse of input data*/

#define CMD02_GET_GLOB_INT_STATUS_ALL                                          \
  0x19 /* Gets the CH395T global interrupt status  */
...
#define CMD60_SET_MAC_ADDR                                                     \
  0x21 /* Set MAC address , Must be set before CMD00H_INIT_CH395 */
       /* Input: 6-byte MAC address */

#define CMD40_SET_IP_ADDR                                                      \
  0x22 /* Set IP address , Must be set before CMD00H_INIT_CH395  */
       /* Input: 4-byte IP address */

#define CMD40_SET_GWIP_ADDR                                                    \
  0x23 /* Set GWIP address , Must be set before CMD00H_INIT_CH395 */
       /* Input: 4-byte GWIP address */

#define CMD40_SET_MASK_ADDR                                                    \
  0x24 /* Set MASK address , Must be set before CMD00H_INIT_CH395 */
       /* Input: 4-byte GWIP address */

#define CMD90_SET_MAC_FILT 0x25 /* Set MAC Filter */
/* Input: 1-byte Preference of Filtering ,4-byte hash 0, 4-byte hash 1*/

#define CMD01_GET_PHY_STATUS                                                   \
  0x26 /* Gets the current PHY status, such as disconnected, FULL/HALF */
/* Output: Current PHY status. See PHY Parameter definition for the status
 * definition */

#define CMD0W_INIT_CH395 0x27 /* Initialize CH395 */
/* This command takes about 200MS to run. This command needs to wait for the
 * command to execute successfully */

#define CMD08_GET_UNREACH_IPPORT 0x28 /* Get unreachable information */
/* Output: 8 bytes, the first byte is the unreachable type, refer to the
 * unreachable type definition */
...
#define CMD01_GET_GLOB_INT_STATUS                                              \
  0x29 /* Gets the CH395 global interrupt status */
/* Output: Global interrupt status, refer to Global interrupt status definition
 */
...
/* Output: Command execution status, refer to the command execution status
 * definition */

#define CMD06_GET_REMOT_IPP_SN                                                 \
  0x2D /* Obtain the port and IP address of the remote end. This command is    \
          used in TCP server mode */
/* Output: 6 bytes. Bytes 1-4 are the IP address of the remote end, and bytes
 * 5-6 are the port number of the remote end */
...
/* Output: First byte: socket n open or closed
   Second byte: TCP state, meaningful only if TCP mode and the first byte is
   open */

#define CMD11_GET_INT_STATUS_SN                                                \
  0x30 /* Gets the interrupt status of socket n                                \
        */
       /* Input: index of the socket */
/* Output: socket interrupt status, refer to socket interrupt status definition
 */

#define CMD50_SET_IP_ADDR_SN                                                   \
  0x31 /* Set the destination IP address of socket n */
/* Input: The first byte is the index value of the socket, Bytes 2-5 are the IP
 * address */

#define CMD30_SET_DES_PORT_SN 0x32 /* Set the destination port of socket n */
/* Input: The first byte is the index value of the socket, Bytes 2-3 are the
 * destination port */
...
/* Input: The first byte is the index value of the socket, the second byte is
 * protocol type, refer to the protocol type definition */

#define CMD1W_OPEN_SOCKET_SN 0x35 /* Open socket n */
/* Input: The first byte is the index value of the socket. This command needs to
 * wait for the command to execute successfully */

#define CMD1W_TCP_LISTEN_SN                                                    \
  0x36 /* socket n listens, After receiving this command, socket n enters      \
          server mode, valid only for TCP mode */
/* Input: The first byte is the index value of the socket. This command needs to
 * wait for the command to execute successfully */

#define CMD1W_TCP_CONNECT_SN                                                   \
  0x37 /* socket n connection. After receiving this command, socket n enters   \
          the client mode, valid only for TCP mode */
/* Input: The first byte is the index value of the socket. This command needs to
 * wait for the command to execute successfully */

#define CMD1W_TCP_DISCONNECT_SN                                                \
  0x38 /* socket n disconnection, valid only for TCP mode */
       /* Input: The first byte is the index value of the socket. */

#define CMD30_WRITE_SEND_BUF_SN 0x39 /* Writes data to socket n buffer */
/* Input: The first byte is the index value of the socket, and the second to
 * third bytes is the length ,And a certain byte of data*/

#define CMD12_GET_RECV_LEN_SN                                                  \
  0x3B /* Gets the length of data received by socket n */
       /* Input: index of the socket */
       /* Output: 2 bytes receive length */

#define CMD30_READ_RECV_BUF_SN 0x3C /* Gets socket n receive buffer data */
/* Input: The first byte is the index value of the socket, and the second to
 * third bytes is the read length n */
/* Output: n data */

#define CMD1W_CLOSE_SOCKET_SN 0x3D /* Close socket n */
                                   /* Input: index of the socket */

#define CMD20_SET_IPRAW_PRO_SN                                                 \
  0x3E /*In IPRAW mode, configure the IP packet protocol field. */
/* Input: The first byte is the index value of the socket, and the second byte
 * is the IP RAW protocol type */
...
/* Output: 20 bytes, respectively 4 bytes IP, 4 bytes gateway, 4 bytes mask, 4
 * bytes DNS1, 4 bytes DNS2 */

#define CMD20_SET_ARP                                                          \
  0x44 /* Set ARP retransmission period and number of times */
/* Input: The first byte is ARP retransmission period, and the second byte is
 * number of ARP times*/

#define CMD20_TCP_MSS 0x50 /* Set TCP MSS */
                           /* Input: 2-byte TCP MSS */

#define CMD20_SET_TTL                                                          \
  0x51 /* Set the TTL. The maximum value of the TTL is 128 */
/* Input: The first byte is the index value of the socket, and the second byte
 * is the TTL value. The maximum value is 128 */

#define CMD30_SET_RECV_BUF 0x52 /* Sets the SOCKET receive buffer */
/* Input: the first byte is the index value of the socket, the second byte is
 * the starting block index, and the third byte is the number of blocks */
...
#define SINT_STAT_RECV                                                         \
  (1 << 2) /* The socket port received data or the receive buffer was not      \
              empty */
#define SINT_STAT_CONNECT                                                      \
  (1 << 3) /* successful connect, This interrupt is generated in TCP mode */
#define SINT_STAT_DISCONNECT                                                   \
  (1 << 4) /* disconnect, This interrupt is generated in TCP mode */
#define SINT_STAT_TIM_OUT                                                      \
  (1 << 6) /* This interrupt is generated in TCP mode                          \
            */
...
/* Function Parameters */
#define FUN_PARA_FLAG_TCP_SERVER                                               \
  (1 << 1) /* TCP server multi-connection mode flag, supported from version    \
              0x44 */
#define SOCK_CTRL_FLAG_SOCKET_CLOSE                                            \
  (1 << 3) /* CH395 does not actively close the socket */
#define SOCK_DISABLE_SEND_OK_INT                                               \
  (1 << 4) /* Send OK interrupt control bit, 1 to disable send OK interrupt */
#define FUN_PARA_FLAG_UART_FAST_DEAL                                           \
  (1 << 7) /* UART fast response enable bit */
#define FUN_PARA_FLAG_UART_FLOW_CONTROL                                        \
  (1 << 8) /* UART flow control enable bit */
...
typedef struct {
  ...
  const ch395_socket_cfg_t *sockets;
  uint8_t socket_count;

  uint8_t ch395_uart_port;
  uint8_t dbg_uart_port;
  uint32_t ch395_baudrate;
} ch395_cfg_t;
...
uint8_t ch395_udp_sendto(uint8_t sockindex, const uint8_t *buf, uint16_t len,
                         const uint8_t *ip, uint16_t port, uint32_t timeout_ms);

uint8_t ch395_udp_sendto_nowait(uint8_t sockindex, const uint8_t *buf,
                                uint16_t len, const uint8_t *ip, uint16_t port);
/* 头文件结束
 * **************************************************************/
#endif /* <CH395>_H */
